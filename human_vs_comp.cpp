		...
		if(board[i][j]==3 || board[i][j]==6)
			{
					int to_x= 0 + (rand() % 7);
					int to_y= 0 + (rand() % 7);
				bool temp =perform_Action_computer(board,i,j,to_x,to_y,next_player);
				if(temp==true)
					break;
			}
		}
...
int main()
{
	...
	while(true)
	{
		...
		if(next_player==0)
		{
			...
			{
				if(next_player==0)
				cout<<" Player1 QUITs, Computer WINS !!!!  "<<endl;
				if(next_player==1)
				cout<<" Computer QUITs, Player1 WINS !!!!  "<<endl;
				break;
			}
			cin>>c;
			int from_y=(int)c-'a';
		...
		{
			cout<<"Computer (white) chance, format:=  from:[row][coloum] to:[row][column], eg, 0a 1b"<<endl;
